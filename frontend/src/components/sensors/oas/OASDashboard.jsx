...
import OASRadar from "../../bridge/OASRadar";
...
const distances = [
latest.forwardDistance ?? 200,
latest.portDistance ?? 200,
latest.starboardDistance ?? 200
];

const minDist = Math.min(...distances);

const risk = Math.max(0, (100 - minDist) / 100);
...
const obstacles = [
{
distance: Math.min(100, latest.forwardDistance ?? 0),
angle: 0,
label: `${latest.forwardDistance ?? "--"}m`
},
{
distance: Math.min(100, latest.portDistance ?? 0),
angle: 270,
label: `${latest.portDistance ?? "--"}m`
},
{
distance: Math.min(100, latest.starboardDistance ?? 0),
angle: 90,
...
<Metric label="Collision Risk" value={collisionRisk} />
...
<circle cx={center} cy={center} r={150} className="sonar-circle" />
